...
# --- Main Scrolling Loop ---
while True:
    # First pass: Right-to-left scrolling
    for TEXT_TO_SCROLL in MSG:
        print(f"Right-to-left: {TEXT_TO_SCROLL}")
    ...
    # Second pass: Left-to-right scrolling
    for TEXT_TO_SCROLL in MSG:
        print(f"Left-to-right: {TEXT_TO_SCROLL}")
        # Calculate the width of the text in pixels
        text_pixel_width = len(TEXT_TO_SCROLL) * 8
        
        # Horizontal scroll loop (left-to-right)
        for x_position in range(-text_pixel_width, DISPLAY_WIDTH + 1):
            display.fill(0) # Clear the entire display buffer for the new frame
